...
class RegressionAlternatorStreamsWithKCLTest(PerformanceRegressionTest):
    """
    Regression test for Alternator's Streams, using KCL based java app consumer.
    The test will spawn cluster, one worker will send requests to Alternator and produce "expected events" file.
    The other worker will run KCL application which will consume the stream and produce "actual events" file.
    After the test is done, the two files will be compared and if they are different, the test will fail.
    """
