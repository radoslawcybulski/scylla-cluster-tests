...
from sdcm.sct_events.loaders import DynamoDBStreamsValidatorStressEvent
...
class DynamoDBStreamsValidatorThread(DockerBasedStressThread):
    ...
    def __init__(self, *args, cluster_tester, **kwargs):
        ...
        LOGGER.debug("Output files directory: %s", self.directory_for_output_files)
        os.makedirs(self.directory_for_output_files, exist_ok=True)
        self.cluster_tester = cluster_tester

    def target_file(self):
        target = os.path.join(self._output_files_directory_on_master_node(), f"output_produced.txt")
        return target
    
    def _output_files_directory_inside_container(self, loader_idx, cpu_idx):
        return f"/tmp/dynamodb-streams-validator-output/{self._uuid_val}/{loader_idx}/{cpu_idx}"
    ...
    def fetch_output_files(self, loader, loader_idx, cpu_idx):
        ...
        source = os.path.join(dr, "output_produced.txt")
        target = self.target_file()
        LOGGER.info(f'downloading output file from loader {loader} with command: cat "{dr}/output_produced.txt" to target {target}')
        try:
    ...
    def _run_stress(self, loader, loader_idx, cpu_idx):  # noqa: PLR0914
        ...
        stress_cmd = 'cd /tmp/dsv && java -jar /app/app.jar'
        ...
        LOGGER.debug("dynamodb-streams-validator local log: %s", log_file_name)

        LOGGER.debug("running: %s", stress_cmd)

        DynamoDBStreamsValidatorStressEvent.start(node=cmd_runner_name, stress_cmd=stress_cmd).publish()

        result = {}
        failure_event = finish_event = None
        LOGGER.debug(f"starting DynamoDBStreamsValidator stress command: {stress_cmd}")
        try:
            LOGGER.debug(f"running DynamoDBStreamsValidator stress command: {stress_cmd}")
            ...
            LOGGER.debug(f"DynamoDBStreamsValidator command finished: {result}")
            self.fetch_output_files()
        except Exception as exc:
            LOGGER.exception(f"DynamoDBStreamsValidator command failed: {exc}")
            errors_str = format_stress_cmd_error(exc)
            failure_event = DynamoDBStreamsValidatorStressEvent.failure(
            ...
            LOGGER.debug("DynamoDBStreamsValidator stress command finished, cleaning up")
            finish_event = DynamoDBStreamsValidatorStressEvent.finish(
                node=cmd_runner_name, stress_cmd=stress_cmd, log_file_name=log_file_name
            )
            finish_event.publish()
        LOGGER.debug("DynamoDBStreamsValidator stress command done")
